...
if(give){
	give.addEventListener('click', function(e){
		...
		if(isEmpty($('#give-iframe'))) {
			$('<iframe>', {
				// src: 'https://online.gifts.washington.edu/secure/?nobanner=true&activateTab=0&appeal=21XBS&code=IMPFND',
...
if(giveNew){
	giveNew.addEventListener('click', function(e){
		...
		if(isEmpty($('#give-iframe .container'))) {
			$('<iframe>', {
				// src: 'https://online.gifts.washington.edu/secure/?nobanner=true&activateTab=0&appeal=21XBS&page=make&code=' + 'IMPFND' + '&amount=' + $("#cv2-header-give-amount").val().replace(/,/g, ''),
